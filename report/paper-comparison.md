...
- Generation CD ~38 vs 논문 34.5 (~10% 차이) — aligned 데이터 정렬로 달성.
- 데이터 절반(851명) + boundary Dice 갭(0.695)에도 불구하고 **생성 품질은 논문 수준.**
- **최종 모델: gen_stage2_aligned** (1차 3000ep + 2차 800ep fine-tune).
  - CD는 gen_aligned(1차)와 동등(평균 39.8 vs 38.7, 10명 비교)이지만, **크라운 형상 품질이 시각적으로 더 자연스러워 사용자 판정으로 stage2 채택.**
- **"논문 재현 실패"가 아닌 "핵심 달성 + 일부 한계(boundary Dice, 데이터 규모)" 상태.**

---

## 부록: 산출물
- **모델**: `gen_stage2_aligned` (**최종 채택**, 1차+2차), `gen_aligned` (1차, CD 35.8), `boundary_aligned` (Dice 0.695)
