...
##################################################
# Below are functions for crown mesh reconstruction

def subsample_supragingival(pcd, num_samples=256, prefer_high_z=True):
    ...
    random_sample_index = np.random.choice(len(pcd), size=num_samples, replace=False, p=p)
    return random_sample_index


def recenter_tooth_sap(pcd):
    centroid = np.mean(pcd, axis=0)
    point_cloud_centered = pcd - centroid

    max_extent = np.max(np.linalg.norm(point_cloud_centered, axis=1))
    scale_factor = 0.55 / max_extent
    point_cloud_scaled = point_cloud_centered * scale_factor

    return point_cloud_scaled, centroid, scale_factor

def export_generated_mesh_crowns(sap_model, dpsr, sap_cfg, device, scenario_save_dir, shift, scale, fdi, generated_tooth_point):

    pcd_np = generated_tooth_point.transpose(1,0) * scale + shift

    num_aug_points = 256
    if fdi > 30:
        pcd_to_aug = pcd_np[subsample_supragingival(pcd_np, prefer_high_z=False, num_samples=num_aug_points)]
    else:
        pcd_to_aug = pcd_np[subsample_supragingival(pcd_np, prefer_high_z=True, num_samples=num_aug_points)]

    pcd_np = np.concatenate([pcd_np, pcd_to_aug], 0)
    pcd_np, centroid, scale_factor = recenter_tooth_sap(pcd_np)
    pcd_np = pcd_np / 1.2 + 0.5

    pcd = torch.from_numpy(np.expand_dims(pcd_np[np.random.randint(0,1024+num_aug_points,1000)], 0)).float().to(device)
    out = sap_model(pcd)
    points, normals = out
    psr_grid = dpsr(points, normals)
    with torch.no_grad():
        v, f, _ = mc_from_psr(psr_grid, 
                    zero_level=sap_cfg['data']['zero_level'])
        ...
        v_transformed = v / scale_factor + centroid

        mesh = trimesh.Trimesh(vertices = v_transformed, faces = f)
        mesh.export(os.path.join(scenario_save_dir, f'gen_tooth_{fdi}.obj'))
