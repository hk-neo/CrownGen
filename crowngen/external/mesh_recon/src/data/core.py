...
class ToothDataset(data.Dataset):
    ...
    def __init__(self, dataset_folder, fields, split=None,
        ...
        split_file = os.path.join(self.dataset_folder, 'meta', 'sap_{}.lst'.format(split))
        with open(split_file, 'r') as f:
            models_c = f.read().split('\n')
        if '' in models_c:
            models_c.remove('')
        self.models += [tooth_path for tooth_path in models_c]

        # for tooth_sap_path in models_c:
        #     pointcloud_data = np.load(os.path.join(self.dataset_folder, tooth_sap_path, 'pointcloud.npz'))
        #     psr_data = np.load(os.path.join(self.dataset_folder, tooth_sap_path, 'psr.npz'))
        #     self.models.append({'pointcloud_data':pointcloud_data, 'psr_data':psr_data})
    ...
    def __getitem__(self, idx):
        ...
        for field_name, field in self.fields.items():
            field_data = field.load_tooth(model_path)
